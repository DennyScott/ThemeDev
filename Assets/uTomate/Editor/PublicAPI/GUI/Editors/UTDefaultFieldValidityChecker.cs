    ...
    public class UTDefaultFieldValidityChecker : UTFieldValidityChecker
    {
		...
		public UTFieldValidity CheckValidity (UTFieldWrapper wrapper, out string errorMessage)
		{
			...
			if (wrapper.IsString) {
				...
				if (wrapper.SupportsExpressions && !wrapper.UseExpression && !wrapper.InspectorHint.containsExpression && wrapper.InspectorHint.displayAs != UTInspectorHint.DisplayAs.Password) {
