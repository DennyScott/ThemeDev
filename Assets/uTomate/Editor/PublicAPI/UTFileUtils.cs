...
namespace AncientLightStudios.uTomate.API
{
    ...
    public class UTFileUtils
    {
        ...
        public static string[] CalculateFileset(string basePath, string[] includes, string[] excludes, FileSelectionMode mode)
        {
            ...
            {
				return new string[0]; // nothing selected, we can save a lot of work.
            }
        ...
        public static string Repath(string path, string oldBasedir, string newBaseDir, bool flatten)
        {
			...
			oldBasedir = ToCanonicalPath (oldBasedir);
			newBaseDir = ToCanonicalPath (newBaseDir);
        ...
        /// <summary>
        /// Checks whether or not the given path represents a file.
        /// </summary>
        ...
        public static bool IsFile(string path)
        {
            ...
            //detect whether its a directory or file (that is NO directory)
        ...
        /// <summary>
        /// Generates a temporary file name and ensures that the path for this file exists. The file name
        /// can then be used to put temporary content somewhere.
        /// </summary>
        /// <param name="extension">the extension of the file (e.g. ".txt"). Can be empty or null.</param>
        /// <returns>the generated temp file path. The file itself is not created.</returns>
        public static String TempFile(string extension)
        {
            var path = NormalizeSlashes( System.IO.Path.GetTempPath() + "/uTomate_Temp/" + Guid.NewGuid().ToString());
		...
		/// <summary>
		/// Converts a path containing "../" etc to a canonical path.
		/// </summary>
        ...
        private static void AddToPaths(HashSet<string> paths, DirectoryInfo root, string selector, FileSelectionMode mode)
        {
            ...
            if (UTPreferences.WarnAboutEmptySelector && selectedPaths.Count == 0)
            {
                Debug.LogWarning("Include selector '" + selector + "' does not match any items within '" + root.FullName + "'.");
            }
        ...
        private static void RemoveFromPaths(HashSet<string> paths, DirectoryInfo root, string selector, FileSelectionMode mode)
        {
            ...
            if (UTPreferences.WarnAboutEmptySelector && selectedPaths.Count == 0)
            {
                Debug.LogWarning("Exclude selector '" + selector + "' does not match any items within '" + root.FullName + "'.");
            }
