...
namespace AncientLightStudios.uTomate
{
    ...
    public class UTEditorProperties
    {
        ...
        public static UTConfigurableProperty AddProperty(string propertyName)
        {
            var result = new UTConfigurableEditorProperty(propertyName, "", "", false, false);
            properties.Add(result);
            PersistCaches();
            return result;
        }

        public static void AddInternal(string propertyName)
        {
            AddProperty(propertyName);
        }

        public static bool TryGetProperty(string propertyName, out UTConfigurableProperty result)
        {
            if (isDirty)
            {
                RebuildCaches();
            }
            result = properties.Find(p => p.Name == propertyName);
            return result != null;
        }
