...
namespace AncientLightStudios.uTomate
{
    ...
    public class UTInternalCall
    {
        ...
        public static void SetStatic(string typeName, string propertyName, object value)
        {
            Type type = GetType(typeName);
            if (type == null)
            {
        ...
        /// <summary>
        /// Invokes a static internal method by reflection. Same as <see cref="InvokeStatic(string,string,object[])"/> but
        ...
        /// Is thrown if the method cannot be successfully invoked.
        /// </exception>
        public static object InvokeStatic(string typeName, string methodName, Type[] parameterTypes, params object[] parameters)
        {
            Type type = GetType(typeName);
            ...
            var method = type.GetMethod(methodName, BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.Public, null, parameterTypes, null);
            if (method == null)
            {
                Debug.LogWarning("Internal method: " + typeName + "." + methodName + " does not exist.", null);
                return null;
        ...
        public static Type GetType(string typeName)
	{
		...
		// If the TypeName is a full name, then we can try loading the defining assembly directly
		    ...
		    var remainingTypeName = typeName.Substring(typeName.IndexOf('.') + 1);

			type = assembly.GetType (remainingTypeName);
		}
